...
*"* do not include other source files here!!!
    INTERFACES zif_xslx2_book_properties .
    INTERFACES zif_xslx2_book_protection .
    INTERFACES zif_xslx2_book_vba_project .

    DATA legacy_palette TYPE REF TO zcl_xslx2_legacy_palette READ-ONLY .
    DATA security TYPE REF TO zcl_xslx2_security .
    DATA use_template TYPE abap_bool .
    CONSTANTS version TYPE c LENGTH 10 VALUE '7.16.0'.      "#EC NOTEXT

    METHODS add_new_autofilter
      IMPORTING
        !io_sheet            TYPE REF TO zcl_xslx2_worksheet
      RETURNING
        VALUE(ro_autofilter) TYPE REF TO zcl_xslx2_autofilter
      RAISING
        zcx_xslx2 .
    METHODS add_new_comment
      RETURNING
        VALUE(eo_comment) TYPE REF TO zcl_xslx2_comment .
    METHODS add_new_drawing
      IMPORTING
        !ip_type          TYPE zxslx2_drawing_type DEFAULT zcl_xslx2_drawing=>type_image
        !ip_title         TYPE clike OPTIONAL
      RETURNING
        VALUE(eo_drawing) TYPE REF TO zcl_xslx2_drawing .
    ...
    METHODS add_new_style
      IMPORTING
        ...
        !io_clone_of    TYPE REF TO zcl_xslx2_style OPTIONAL
          PREFERRED PARAMETER !ip_guid
      RETURNING
        VALUE(eo_style) TYPE REF TO zcl_xslx2_style .
    METHODS add_new_worksheet
      IMPORTING
        !ip_title           TYPE zxslx2_sheet_title OPTIONAL
      RETURNING
        VALUE(eo_worksheet) TYPE REF TO zcl_xslx2_worksheet
      RAISING
        zcx_xslx2 .
    ...
    METHODS get_active_sheet_index
      RETURNING
        VALUE(r_active_worksheet) TYPE zxslx2_active_worksheet .
    METHODS get_active_worksheet
      RETURNING
        VALUE(eo_worksheet) TYPE REF TO zcl_xslx2_worksheet .
    METHODS get_autofilters_reference
      RETURNING
        VALUE(ro_autofilters) TYPE REF TO zcl_xslx2_autofilters .
    ...
    METHODS get_static_cellstyle_guid
      IMPORTING
        !ip_cstyle_complete  TYPE zxslx2_s_cstyle_complete
        !ip_cstylex_complete TYPE zxslx2_s_cstylex_complete
      RETURNING
    ...
    METHODS get_style_to_guid
      IMPORTING
        !ip_guid               TYPE zxslx2_cell_style
      RETURNING
        VALUE(ep_stylemapping) TYPE zxslx2_s_stylemapping
      RAISING
        zcx_xslx2 .
    METHODS get_theme
      EXPORTING
        !eo_theme TYPE REF TO zcl_xslx2_theme .
    METHODS get_worksheets_iterator
      RETURNING
        VALUE(eo_iterator) TYPE REF TO zcl_xslx2_collection_iterator .
    METHODS get_worksheets_name
      RETURNING
        VALUE(ep_name) TYPE zxslx2_worksheets_name .
    ...
    METHODS get_worksheet_by_name
      IMPORTING
        !ip_sheet_name      TYPE zxslx2_sheet_title
      RETURNING
        VALUE(eo_worksheet) TYPE REF TO zcl_xslx2_worksheet .
    METHODS set_active_sheet_index
      IMPORTING
        !i_active_worksheet TYPE zxslx2_active_worksheet
      RAISING
        zcx_xslx2 .
    METHODS set_active_sheet_index_by_name
      IMPORTING
        !i_worksheet_name TYPE zxslx2_worksheets_name .
    ...
    METHODS set_theme
      IMPORTING
        !io_theme TYPE REF TO zcl_xslx2_theme .
    METHODS fill_template
      IMPORTING
        !iv_data TYPE REF TO zcl_xslx2_template_data
      RAISING
        zcx_xslx2 .
  PROTECTED SECTION.

    DATA worksheets TYPE REF TO zcl_xslx2_worksheets .
  PRIVATE SECTION.

    DATA autofilters TYPE REF TO zcl_xslx2_autofilters .
    DATA charts TYPE REF TO zcl_xslx2_drawings .
    DATA default_style TYPE zxslx2_cell_style .
...
*"* do not include other source files here!!!
    ...
    DATA styles TYPE REF TO zcl_xslx2_styles .
    DATA t_stylemapping1 TYPE zxslx2_t_stylemapping1 .
    DATA t_stylemapping2 TYPE zxslx2_t_stylemapping2 .
    DATA theme TYPE REF TO zcl_xslx2_theme .
    DATA comments TYPE REF TO zcl_xslx2_comments .

    METHODS stylemapping_dynamic_style
      IMPORTING
        !ip_style        TYPE REF TO zcl_xslx2_style
      RETURNING
        VALUE(eo_style2) TYPE zxslx2_s_stylemapping .
ENDCLASS.



CLASS zcl_xslx2 IMPLEMENTATION.
...
* Create default style
    ...
    DATA: style2 TYPE zxslx2_s_stylemapping.
...
* Create default blank worksheet
  ...
  METHOD add_static_styles.
    ...
    DATA: style TYPE REF TO zcl_xslx2_style.
    ...
    LOOP AT me->t_stylemapping1 ASSIGNING <style1> USING KEY added_to_iterator WHERE added_to_iterator = abap_false.
      ...
      zcl_xslx2_common=>recursive_struct_to_class( EXPORTING i_source  = <style1>-complete_style
...
* Inizialize instance objects
    ...
    me->zif_xslx2_book_protection~initialize( ).
    me->zif_xslx2_book_properties~initialize( ).

    TRY.
        me->add_new_worksheet( ).
      CATCH zcx_xslx2. " suppress syntax check error
    ...
    lo_style->fill->filltype = zcl_xslx2_style_fill=>c_fill_pattern_gray125.
  ...
  METHOD delete_worksheet.
    ...
    DATA: lo_worksheet    TYPE REF TO zcl_xslx2_worksheet,
          l_size          TYPE i,
  ...
  METHOD fill_template.
    ...
    DATA: lo_template_filler TYPE REF TO zcl_xslx2_fill_template.

    FIELD-SYMBOLS:
      <lv_sheet>     TYPE zxslx2_sheet_title,
      <lv_data_line> TYPE zcl_xslx2_template_data=>ts_template_data_sheet.


    lo_template_filler = zcl_xslx2_fill_template=>create( me ).
  ...
  METHOD get_static_cellstyle_guid.
    ...
    IF sy-subrc <> 0.
      ...
      style-guid = zcl_xslx2_obsolete_func_wrap=>guid_create( ). " ins issue #379 - replacement for outdated function call
  ...
  METHOD get_style_index_in_styles.
    ...
    IF ep_index IS INITIAL.
      zcx_xslx2=>raise_text( 'Index not found' ).
    ELSE.
  ...
  METHOD get_style_to_guid.
    ...
    IF sy-subrc <> 0.
      zcx_xslx2=>raise_text( 'GUID not found' ).
    ENDIF.

    IF ep_stylemapping-dynamic_style_guid IS NOT INITIAL.
      lo_style = me->get_style_from_guid( ip_guid ).
      zcl_xslx2_common=>recursive_class_to_struct( EXPORTING i_source = lo_style
  ...
  METHOD get_worksheet_by_index.
    ...
    DATA: lv_index TYPE zxslx2_active_worksheet.
...
*--------------------------------------------------------------------*
  ...
  METHOD set_active_sheet_index_by_name.
    ...
    DATA: ws_it    TYPE REF TO zcl_xslx2_collection_iterator,
          ws       TYPE REF TO zcl_xslx2_worksheet,
          lv_title TYPE zxslx2_sheet_title,
...
* dynamically
  ...
  METHOD zif_xslx2_book_properties~initialize.
    DATA: lv_timestamp TYPE timestampl.

    me->zif_xslx2_book_properties~application     = 'Microsoft Excel'.
    me->zif_xslx2_book_properties~appversion      = '12.0000'.

    GET TIME STAMP FIELD lv_timestamp.
    me->zif_xslx2_book_properties~created         = lv_timestamp.
    me->zif_xslx2_book_properties~creator         = sy-uname.
    me->zif_xslx2_book_properties~description     = zcl_xslx2=>version.
    me->zif_xslx2_book_properties~modified        = lv_timestamp.
    me->zif_xslx2_book_properties~lastmodifiedby  = sy-uname.
  ENDMETHOD.


  METHOD zif_xslx2_book_protection~initialize.
    me->zif_xslx2_book_protection~protected      = zif_xslx2_book_protection=>c_unprotected.
    me->zif_xslx2_book_protection~lockrevision   = zif_xslx2_book_protection=>c_unlocked.
    me->zif_xslx2_book_protection~lockstructure  = zif_xslx2_book_protection=>c_unlocked.
    me->zif_xslx2_book_protection~lockwindows    = zif_xslx2_book_protection=>c_unlocked.
    CLEAR me->zif_xslx2_book_protection~workbookpassword.
    CLEAR me->zif_xslx2_book_protection~revisionspassword.
  ENDMETHOD.


  METHOD zif_xslx2_book_vba_project~set_codename.
    me->zif_xslx2_book_vba_project~codename = ip_codename.
  ENDMETHOD.


  METHOD zif_xslx2_book_vba_project~set_codename_pr.
    me->zif_xslx2_book_vba_project~codename_pr = ip_codename_pr.
  ENDMETHOD.


  METHOD zif_xslx2_book_vba_project~set_vbaproject.
    me->zif_xslx2_book_vba_project~vbaproject = ip_vbaproject.
  ENDMETHOD.
ENDCLASS.
