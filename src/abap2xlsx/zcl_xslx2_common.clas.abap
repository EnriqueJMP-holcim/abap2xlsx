CLASS zcl_xslx2_common DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
*"* do not include other source files here!!!
  PUBLIC SECTION.
    ...
    CLASS-METHODS convert_column2alpha
      ...
      RETURNING
        VALUE(ep_column) TYPE zxslx2_cell_column_alpha
      RAISING
        zcx_xslx2 .
    CLASS-METHODS convert_column2int
      IMPORTING
        !ip_column       TYPE simple
      RETURNING
        VALUE(ep_column) TYPE zxslx2_cell_column
      RAISING
        zcx_xslx2 .
    ...
    CLASS-METHODS convert_columnrow2column_a_row
      ...
      EXPORTING
        !e_column     TYPE zxslx2_cell_column_alpha
        !e_column_int TYPE zxslx2_cell_column
        !e_row        TYPE zxslx2_cell_row
      RAISING
        zcx_xslx2.
    CLASS-METHODS convert_range2column_a_row
      ...
      EXPORTING
        !e_column_start     TYPE zxslx2_cell_column_alpha
        !e_column_start_int TYPE zxslx2_cell_column
        !e_column_end       TYPE zxslx2_cell_column_alpha
        !e_column_end_int   TYPE zxslx2_cell_column
        !e_row_start        TYPE zxslx2_cell_row
        !e_row_end          TYPE zxslx2_cell_row
        !e_sheet            TYPE clike
      RAISING
        zcx_xslx2 .
    ...
    CLASS-METHODS date_to_excel_string
      IMPORTING
        !ip_value       TYPE d
      RETURNING
        VALUE(ep_value) TYPE zxslx2_cell_value .
    CLASS-METHODS encrypt_password
      IMPORTING
        !i_pwd                 TYPE zxslx2_aes_password
      RETURNING
        VALUE(r_encrypted_pwd) TYPE zxslx2_aes_password .
    ...
    CLASS-METHODS excel_string_to_date
      IMPORTING
        !ip_value       TYPE zxslx2_cell_value
    ...
    CLASS-METHODS get_fieldcatalog
      ...
      RETURNING
        VALUE(ep_fieldcatalog) TYPE zxslx2_t_fieldcatalog .
...
*"* do not include other source files here!!!
  ...
  PRIVATE SECTION.
    ...
    CLASS-DATA sv_prev_in1  TYPE zxslx2_cell_column.
    CLASS-DATA sv_prev_out1 TYPE zxslx2_cell_column_alpha.
    CLASS-DATA sv_prev_in2  TYPE c LENGTH 10.
    CLASS-DATA sv_prev_out2 TYPE zxslx2_cell_column.
    ...
    CLASS-METHODS char2hex
      ...
      RETURNING
        VALUE(r_hex) TYPE zxslx2_pwd_hash .
    CLASS-METHODS shl01
      IMPORTING
        !i_pwd_hash       TYPE zxslx2_pwd_hash
      RETURNING
        VALUE(r_pwd_hash) TYPE zxslx2_pwd_hash .
    CLASS-METHODS shr14
      IMPORTING
        !i_pwd_hash       TYPE zxslx2_pwd_hash
      RETURNING
...
CLASS zcl_xslx2_common IMPLEMENTATION.


  METHOD calculate_cell_distance.

    DATA: lv_reference_row       TYPE i,
          lv_reference_col_alpha TYPE zxslx2_cell_column_alpha,
          lv_reference_col       TYPE i,
          lv_current_row         TYPE i,
          lv_current_col_alpha   TYPE zxslx2_cell_column_alpha,
...
*--------------------------------------------------------------------*
    IF   lv_column > 16384
      OR lv_column < 1.
      zcx_xslx2=>raise_text( 'Index out of bounds' ).
    ENDIF.
...
*--------------------------------------------------------------------*
    ...
    DATA: lv_column       TYPE zxslx2_cell_column_alpha,
...
*--------------------------------------------------------------------*
    ...
    IF lv_column_c EQ ''.
      MESSAGE e800(zabapxslx2) INTO lv_errormessage.
      zcx_xslx2=>raise_symsg( ).
    ENDIF.
...
*--------------------------------------------------------------------*
  ...
  METHOD convert_column_a_row2columnrow.
    ...
    lv_row_alpha = i_row.
    lv_column_alpha = zcl_xslx2_common=>convert_column2alpha( i_column ).
...
* Initialize output parameters
  ...
  METHOD date_to_excel_string.
    ...
    ENDIF.
    ep_value = zcl_xslx2_common=>number_to_excel_string( ip_value = lv_date_diff ).
  ENDMETHOD.
...
*--------------------------------------------------------------------*
  ...
  METHOD encrypt_password.
    ...
    DATA lv_curr_hex               TYPE zxslx2_pwd_hash.
    DATA lv_pwd_len                TYPE zxslx2_pwd_hash.
    DATA lv_pwd_hash               TYPE zxslx2_pwd_hash.

    CONSTANTS:
      lv_0x7fff TYPE zxslx2_pwd_hash VALUE '7FFF',
      lv_0x0001 TYPE zxslx2_pwd_hash VALUE '0001',
      lv_0xce4b TYPE zxslx2_pwd_hash VALUE 'CE4B'.

    DATA lv_pwd            TYPE zxslx2_aes_password.
  ...
  METHOD excel_string_to_date.
    ...
    TRY.
        ...
        IF lv_date_int NOT BETWEEN 1 AND 2958465.
          zcx_xslx2=>raise_text( 'Unable to interpret date' ).
        ENDIF.
      ...
      CATCH cx_sy_conversion_error.
        ...
        zcx_xslx2=>raise_text( lv_error_text ).
    ENDTRY.
  ENDMETHOD.
...
* But currently this works fine - even for numbers in scientific notation
  ...
  METHOD excel_string_to_time.
    ...
    TRY.
      ...
      CATCH cx_sy_conversion_error.
        zcx_xslx2=>raise_text( 'Unable to interpret time' ).
    ENDTRY.
  ENDMETHOD.
...
* just use the fieldname as description - that is better than nothing
  ...
  METHOD is_cell_in_range.
    DATA lv_column_start    TYPE zxslx2_cell_column_alpha.
    DATA lv_column_end      TYPE zxslx2_cell_column_alpha.
    DATA lv_row_start       TYPE zxslx2_cell_row.
    DATA lv_row_end         TYPE zxslx2_cell_row.
    DATA lv_column_start_i  TYPE zxslx2_cell_column.
    DATA lv_column_end_i    TYPE zxslx2_cell_column.
    DATA lv_column_i        TYPE zxslx2_cell_column.
...
* not - then it is an attribute of the class - use different assign then
      ...
      CASE wa_component-type_kind.
        WHEN cl_abap_structdescr=>typekind_struct1 OR cl_abap_structdescr=>typekind_struct2.  " Structure --> use recursio
          zcl_xslx2_common=>recursive_class_to_struct( EXPORTING i_source  = <attribute>
...
* This is private an no one using it so far except me, so no need to hurry
    DATA: descr          TYPE REF TO cl_abap_structdescr,
          ...
          attribute_name LIKE wa_component-name,
          flag_class     TYPE abap_bool,
          o_border       TYPE REF TO zcl_xslx2_style_border.
...
* Only borders will be passed as unbound references.  But since we want to set a value we have to create an instance
            ...
            <attribute> = o_border.
          ENDIF.
          zcl_xslx2_common=>recursive_struct_to_class( EXPORTING i_source  = <field>
                                                                 i_sourcex = <fieldx>
...
*--------------------------------------------------------------------*
          TRY.
              lv_tcoln = zcl_xslx2_common=>convert_column2int( lv_tcol1 ) + iv_shift_cols.
            CATCH zcx_xslx2.
              CONCATENATE lv_cur_form lv_substr1 INTO lv_cur_form.
              lv_cnt = lv_cnt + 1.
              lv_offset1 = lv_cnt.
...
*--------------------------------------------------------------------*
            ...
            ELSE.
              TRY.
                  lv_tcol2 = zcl_xslx2_common=>convert_column2alpha( lv_tcoln ).
...
* We skip it. for now.
  ...
  METHOD time_to_excel_string.
    ...
    ep_value = zcl_xslx2_common=>number_to_excel_string( ip_value = lv_day_fraction ).
  ENDMETHOD.
