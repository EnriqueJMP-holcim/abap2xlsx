CLASS zcl_xslx2_converter_salv_table DEFINITION
  PUBLIC
  INHERITING FROM zcl_xslx2_converter_alv
  FINAL
  CREATE PUBLIC .
...
*"* do not include other source files here!!!
  PUBLIC SECTION.
    ...
    METHODS zif_xslx2_converter~can_convert_object
        REDEFINITION .
    METHODS zif_xslx2_converter~create_fieldcatalog
        REDEFINITION .
    METHODS zif_xslx2_converter~get_supported_class
        REDEFINITION .
...
CLASS zcl_xslx2_converter_salv_table IMPLEMENTATION.
...
* First update metadata if we can.
    IF io_salv->is_offline( ) = abap_false.
      ...
      ELSE.
        IF zcl_xslx2_converter_salv_model=>is_get_metadata_callable( io_salv ) = abap_true.
...
*  ... get the filter information
  ...
  METHOD zif_xslx2_converter~get_supported_class.
    rv_supported_class = 'CL_SALV_TABLE'.
  ENDMETHOD.

  METHOD zif_xslx2_converter~can_convert_object.
  ...
  METHOD zif_xslx2_converter~create_fieldcatalog.
    DATA: lo_salv TYPE REF TO cl_salv_table.

    zif_xslx2_converter~can_convert_object( io_object = io_object ).
