CLASS zcl_xslx2_fill_template DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES: tv_range_name TYPE c LENGTH 30,
           BEGIN OF ts_range,
             sheet  TYPE zxslx2_sheet_title,
             name   TYPE tv_range_name,
             start  TYPE zxslx2_cell_row,
             stop   TYPE zxslx2_cell_row,
             id     TYPE i,
             parent TYPE i,
             length TYPE i,
           END OF ts_range,
           ...
           tt_sheet_titles TYPE STANDARD TABLE OF zxslx2_sheet_title WITH DEFAULT KEY,
    ...
    DATA mo_excel TYPE REF TO zcl_xslx2 READ-ONLY.
    ...
    CLASS-METHODS create
      ...
      RETURNING
        VALUE(eo_template_filler) TYPE REF TO zcl_xslx2_fill_template
      RAISING
        zcx_xslx2.

    METHODS fill_sheet
      IMPORTING
        !iv_data TYPE zcl_xslx2_template_data=>ts_template_data_sheet
      RAISING
        zcx_xslx2 .
  ...
  PRIVATE SECTION.
    ...
    TYPES: tt_cell_data_no_key TYPE STANDARD TABLE OF zxslx2_s_cell_data WITH DEFAULT KEY.

    METHODS fill_range
      IMPORTING
        !iv_sheet              TYPE zxslx2_sheet_title
        !iv_parent             TYPE zxslx2_cell_row
        !iv_data               TYPE data
        VALUE(iv_range_length) TYPE zxslx2_cell_row
        !io_sheet              TYPE REF TO zcl_xslx2_worksheet
      CHANGING
        !ct_cells              TYPE tt_cell_data_no_key
        !cv_diff               TYPE zxslx2_cell_row
        !ct_merged_cells       TYPE zcl_xslx2_worksheet=>mty_ts_merge
      RAISING
        zcx_xslx2 .
    METHODS get_range .
    METHODS validate_range
      IMPORTING
        !io_range TYPE REF TO zcl_xslx2_range .
...
CLASS zcl_xslx2_fill_template IMPLEMENTATION.
  ...
  METHOD fill_range.
    ...
    DATA: lt_tmp_cells_template        TYPE tt_cell_data_no_key,
          ...
          ls_cell                      TYPE zxslx2_s_cell_data,
          lt_tmp_merged_cells_template TYPE zcl_xslx2_worksheet=>mty_ts_merge,
          lt_merged_cells_result       TYPE zcl_xslx2_worksheet=>mty_ts_merge,
          lt_tmp_merged_cells          TYPE zcl_xslx2_worksheet=>mty_ts_merge,
...
*      update cells before any range
      ...
      LOOP AT ct_cells INTO ls_cell  WHERE cell_row >= lv_start_row AND cell_row <= lv_stop_row .
        ...
        lv_column_alpha = zcl_xslx2_common=>convert_column2alpha( ls_cell-cell_column ).
...
*      replace variables of current range with data
  ...
  METHOD fill_sheet.
    ...
    DATA: lo_worksheet      TYPE REF TO zcl_xslx2_worksheet,
          ...
          lt_merged_cells   TYPE zcl_xslx2_worksheet=>mty_ts_merge,
          lt_merged_cells_2 TYPE zcl_xslx2_worksheet=>mty_ts_merge,
          lv_initial_diff   TYPE i.
    FIELD-SYMBOLS:
      <any_data>       TYPE any,
      <ls_sheet_cell>  TYPE zxslx2_s_cell_data,
      <ls_merged_cell> TYPE zcl_xslx2_worksheet=>mty_merge.
  ...
  METHOD find_var.
    ...
    DATA: lv_row            TYPE i,
          ...
          lv_highest_column TYPE zxslx2_cell_column,
    ...
    LOOP AT mt_sheet ASSIGNING <lv_sheet>.
      ...
      WHILE lv_row <= lv_highest_row.
        ...
        WHILE lv_column <= lv_highest_column.
          lv_column_alpha = zcl_xslx2_common=>convert_column2alpha( lv_column ).
          CLEAR lo_style.
  ...
  METHOD get_range.
    ...
    DATA:
      lo_worksheets_iterator TYPE REF TO zcl_xslx2_collection_iterator,
      lo_worksheet           TYPE REF TO zcl_xslx2_worksheet,
      lo_range_iterator      TYPE REF TO zcl_xslx2_collection_iterator,
