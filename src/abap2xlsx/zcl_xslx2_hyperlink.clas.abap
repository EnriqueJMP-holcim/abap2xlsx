CLASS zcl_xslx2_hyperlink DEFINITION
  PUBLIC
  FINAL
  CREATE PRIVATE .
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS create_external_link
      IMPORTING
        !iv_url        TYPE string
      RETURNING
        VALUE(ov_link) TYPE REF TO zcl_xslx2_hyperlink .
...
CLASS zcl_xslx2_hyperlink IMPLEMENTATION.
  ...
  METHOD create_external_link.
    ...
    ov_link = zcl_xslx2_hyperlink=>create( iv_url = iv_url
                                           iv_internal = abap_false ).
  ENDMETHOD.


  METHOD create_internal_link.
    ov_link = zcl_xslx2_hyperlink=>create( iv_url = iv_location
  ...
  METHOD set_cell_reference.
    me->column = zcl_xslx2_common=>convert_column2alpha( ip_column ). " issue #155 - less restrictive typing for ip_column
    me->row = ip_row.
  ENDMETHOD.
ENDCLASS.
