CLASS zcl_xslx2_row DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_xslx2_row IMPLEMENTATION.
  ...
  METHOD get_collapsed.
    ...
    DATA: lt_row_outlines  TYPE zcl_xslx2_worksheet=>mty_ts_outlines_row,
...
* Maybe we can use the outline information in the worksheet
  ...
  METHOD set_outline_level.
    IF   ip_outline_level < 0
      ...
      zcx_xslx2=>raise_text( 'Outline level must range between 0 and 7.' ).
  ...
  METHOD set_row_height.
    ...
    TRY.
        ...
        IF height <= 0.
          zcx_xslx2=>raise_text( 'Please supply a positive number as row-height' ).
        ENDIF.
        me->row_height = ip_row_height.
      CATCH cx_sy_conversion_no_number.
        zcx_xslx2=>raise_text( 'Unable to interpret ip_row_height as number' ).
    ENDTRY.
