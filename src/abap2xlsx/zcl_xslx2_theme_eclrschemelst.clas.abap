CLASS zcl_xslx2_theme_eclrschemelst DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_xslx2_theme_eclrschemelst IMPLEMENTATION.
  ...
  METHOD build_xml.
    ...
    IF extracolor IS INITIAL.
      lo_theme_objdef ?= io_document->create_simple_element_ns( prefix = zcl_xslx2_theme=>c_theme_prefix
                                                                name   = zcl_xslx2_theme=>c_theme_extra_color
                                                                parent = lo_theme ).
  ...
  METHOD load.
    extracolor = zcl_xslx2_common=>clone_ixml_with_namespaces( io_extra_color ).
  ENDMETHOD.                    "load
ENDCLASS.
