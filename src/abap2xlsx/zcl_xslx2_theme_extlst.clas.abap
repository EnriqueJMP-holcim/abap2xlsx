CLASS zcl_xslx2_theme_extlst DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_xslx2_theme_extlst IMPLEMENTATION.
  ...
  METHOD build_xml.
    ...
    IF extlst IS INITIAL.
      lo_theme_objdef ?= io_document->create_simple_element_ns( prefix = zcl_xslx2_theme=>c_theme_prefix
                                                                name   = zcl_xslx2_theme=>c_theme_extlst
                                                                parent = lo_theme ).
  ...
  METHOD load.
    extlst = zcl_xslx2_common=>clone_ixml_with_namespaces( io_extlst ).
  ENDMETHOD.                    "load
ENDCLASS.
