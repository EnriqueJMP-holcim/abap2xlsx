CLASS zcl_xslx2_theme_objectdefaults DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_xslx2_theme_objectdefaults IMPLEMENTATION.
  ...
  METHOD build_xml.
    ...
    IF objectdefaults IS INITIAL.
      lo_theme_objdef ?= io_document->create_simple_element_ns( prefix = zcl_xslx2_theme=>c_theme_prefix
                                                                name   = zcl_xslx2_theme=>c_theme_object_def
                                                                parent = lo_theme ).
    ELSE.
  ...
  METHOD load.
    objectdefaults = zcl_xslx2_common=>clone_ixml_with_namespaces( io_object_def ).
  ENDMETHOD.                    "load
ENDCLASS.
