CLASS zcl_xslx2_writer_2007 DEFINITION
  PUBLIC
  CREATE PUBLIC .
...
*"* do not include other source files here!!!
    TYPES: BEGIN OF mty_column_formula_used,
             id TYPE zxslx2_s_cell_data-column_formula_id,
             si TYPE string,
             "! type: shared, etc.
             t  TYPE string,
    ...
    DATA excel TYPE REF TO zcl_xslx2 .
    DATA shared_strings TYPE zxslx2_t_shared_string .
    DATA styles_cond_mapping TYPE zxslx2_t_styles_cond_mapping .
    DATA styles_mapping TYPE zxslx2_t_styles_mapping .
    ...
    METHODS create_dxf_style
      IMPORTING
        !iv_cell_style    TYPE zxslx2_cell_style
        !io_dxf_element   TYPE REF TO if_ixml_element
        !io_ixml_document TYPE REF TO if_ixml_document
        !it_cellxfs       TYPE zxslx2_t_cellxfs
        !it_fonts         TYPE zxslx2_t_style_font
        !it_fills         TYPE zxslx2_t_style_fill
      CHANGING
        !cv_dfx_count     TYPE i .
    ...
    METHODS create_xl_charts
      IMPORTING
        !io_drawing       TYPE REF TO zcl_xslx2_drawing
      RETURNING
        VALUE(ep_content) TYPE xstring .
    ...
    METHODS create_xl_styles_font_node
      IMPORTING
        !io_document TYPE REF TO if_ixml_document
        !io_parent   TYPE REF TO if_ixml_element
        !is_font     TYPE zxslx2_s_style_font
        !iv_use_rtf  TYPE abap_bool DEFAULT abap_false .
    METHODS create_xl_table
      IMPORTING
        !io_table         TYPE REF TO zcl_xslx2_table
      RETURNING
        VALUE(ep_content) TYPE xstring
      RAISING
        zcx_xslx2 .
    ...
    METHODS get_shared_string_index
      IMPORTING
        !ip_cell_value  TYPE zxslx2_cell_value
    ...
    METHODS escape_string_value
      IMPORTING
        !iv_value     TYPE zxslx2_cell_value
      RETURNING
        VALUE(result) TYPE zxslx2_cell_value.
    METHODS set_vml_shape_footer
      IMPORTING
        !is_footer        TYPE zxslx2_s_worksheet_head_foot
      RETURNING
        VALUE(ep_content) TYPE string .
    METHODS set_vml_shape_header
      IMPORTING
        !is_header        TYPE zxslx2_s_worksheet_head_foot
      RETURNING
        VALUE(ep_content) TYPE string .
    ...
    METHODS create_xl_sheet_column_formula
      IMPORTING
        io_document             TYPE REF TO if_ixml_document
        it_column_formulas      TYPE zcl_xslx2_worksheet=>mty_th_column_formula
        is_sheet_content        TYPE zxslx2_s_cell_data
      EXPORTING
...
CLASS zcl_xslx2_writer_2007 IMPLEMENTATION.
...
* Office 2007 file format is a cab of several xml files with extension .xlsx
    ...
    DATA: lo_zip              TYPE REF TO cl_abap_zip,
          ...
          lo_active_worksheet TYPE REF TO zcl_xslx2_worksheet,
          lo_iterator         TYPE REF TO zcl_xslx2_collection_iterator,
          lo_nested_iterator  TYPE REF TO zcl_xslx2_collection_iterator,
          lo_table            TYPE REF TO zcl_xslx2_table,
          lo_drawing          TYPE REF TO zcl_xslx2_drawing.
...
**********************************************************************
* STEP 11: Add media
    lo_iterator = me->excel->get_drawings_iterator( zcl_xslx2_drawing=>type_image ).
    WHILE lo_iterator->has_next( ) EQ abap_true.
...
**********************************************************************
* STEP 12: Add charts
    lo_iterator = me->excel->get_drawings_iterator( zcl_xslx2_drawing=>type_chart ).
    WHILE lo_iterator->has_next( ) EQ abap_true.
      lo_drawing ?= lo_iterator->get_next( ).
...
* Begin - Add - GGAR
      " Comments
      DATA: lo_comments TYPE REF TO zcl_xslx2_comments.
...
* End   - Add - GGAR
      ...
      " Drawings
      DATA: lo_drawings TYPE REF TO zcl_xslx2_drawings.
...
* STEP 4: Create subnodes
    ...
    lo_element = lo_document->create_simple_element( name   = lc_xml_node_application
                                                     parent = lo_document ).
    lv_value = excel->zif_xslx2_book_properties~application.
    ...
    lo_element = lo_document->create_simple_element( name   = lc_xml_node_docsecurity
                                                              parent = lo_document ).
    lv_value = excel->zif_xslx2_book_properties~docsecurity.
    lo_element->set_value( value = lv_value ).
    lo_element_root->append_child( new_child = lo_element ).
    ...
    lo_element = lo_document->create_simple_element( name   = lc_xml_node_scalecrop
                                                     parent = lo_document ).
    lv_value = me->flag2bool( excel->zif_xslx2_book_properties~scalecrop ).
    lo_element->set_value( value = lv_value ).
    lo_element_root->append_child( new_child = lo_element ).
    ...
    " Company
    IF excel->zif_xslx2_book_properties~company IS NOT INITIAL.
      lo_element = lo_document->create_simple_element( name   = lc_xml_node_company
                                                       parent = lo_document ).
      lv_value = excel->zif_xslx2_book_properties~company.
      lo_element->set_value( value = lv_value ).
      lo_element_root->append_child( new_child = lo_element ).
    ENDIF.
    ...
    lo_element = lo_document->create_simple_element( name   = lc_xml_node_linksuptodate
                                                     parent = lo_document ).
    lv_value = me->flag2bool( excel->zif_xslx2_book_properties~linksuptodate ).
    lo_element->set_value( value = lv_value ).
    lo_element_root->append_child( new_child = lo_element ).
    ...
    lo_element = lo_document->create_simple_element( name   = lc_xml_node_shareddoc
                                                     parent = lo_document ).
    lv_value = me->flag2bool( excel->zif_xslx2_book_properties~shareddoc ).
    lo_element->set_value( value = lv_value ).
    lo_element_root->append_child( new_child = lo_element ).
    ...
    lo_element = lo_document->create_simple_element( name   = lc_xml_node_hyperlinkschanged
                                                     parent = lo_document ).
    lv_value = me->flag2bool( excel->zif_xslx2_book_properties~hyperlinkschanged ).
    lo_element->set_value( value = lv_value ).
    lo_element_root->append_child( new_child = lo_element ).
    ...
    lo_element = lo_document->create_simple_element( name   = lc_xml_node_appversion
                                                     parent = lo_document ).
    lv_value = excel->zif_xslx2_book_properties~appversion.
    lo_element->set_value( value = lv_value ).
    lo_element_root->append_child( new_child = lo_element ).
...
* STEP 4: Create subnodes
    ...
    lo_element = lo_document->create_simple_element_ns( name   = lc_xml_node_creator
                                                        prefix = lc_dc_ns
                                                        parent = lo_document ).
    lv_value = excel->zif_xslx2_book_properties~creator.
    lo_element->set_value( value = lv_value ).
    lo_element_root->append_child( new_child = lo_element ).
    ...
    lo_element = lo_document->create_simple_element_ns( name   = lc_xml_node_description
                                                        prefix = lc_dc_ns
                                                        parent = lo_document ).
    lv_value = excel->zif_xslx2_book_properties~description.
    lo_element->set_value( value = lv_value ).
    lo_element_root->append_child( new_child = lo_element ).
    ...
    lo_element = lo_document->create_simple_element_ns( name   = lc_xml_node_lastmodifiedby
                                                        prefix = lc_cp_ns
                                                        parent = lo_document ).
    lv_value = excel->zif_xslx2_book_properties~lastmodifiedby.
    lo_element->set_value( value = lv_value ).
    lo_element_root->append_child( new_child = lo_element ).
    ...
    CONVERT TIME STAMP excel->zif_xslx2_book_properties~created TIME ZONE sy-zonlo INTO DATE lv_date TIME lv_time.
    ...
    lo_element->set_attribute_ns( name    = lc_xml_attr_type
                                  prefix  = lc_xsi_ns
                                  value   = lc_xml_attr_target ).
    CONVERT TIME STAMP excel->zif_xslx2_book_properties~modified TIME ZONE sy-zonlo INTO DATE lv_date TIME lv_time.
    CONCATENATE lv_date lv_time INTO lv_value RESPECTING BLANKS.
    REPLACE ALL OCCURRENCES OF REGEX '([0-9]{4})([0-9]{2})([0-9]{2})([0-9]{2})([0-9]{2})([0-9]{2})' IN lv_value WITH '$1-$2-$3T$4:$5:$6Z'.
    lo_element->set_value( value = lv_value ).
    lo_element_root->append_child( new_child = lo_element ).
...
* STEP 5: Create xstring stream
  ...
  METHOD create_dxf_style.
    ...
    DATA: ls_styles_mapping     TYPE zxslx2_s_styles_mapping,
          ls_cellxfs            TYPE zxslx2_s_cellxfs,
          ls_style_cond_mapping TYPE zxslx2_s_styles_cond_mapping,
          ...
          lv_value              TYPE string,
          ls_fill               TYPE zxslx2_s_style_fill,
    ...
    ELSE.
      ...
      IF ls_fill IS NOT INITIAL.
        ...
        IF  ls_fill-fgcolor-rgb IS INITIAL AND
          ls_fill-fgcolor-indexed EQ zcl_xslx2_style_color=>c_indexed_not_set AND
          ls_fill-fgcolor-theme EQ zcl_xslx2_style_color=>c_theme_not_set AND
          ls_fill-fgcolor-tint IS INITIAL AND ls_fill-bgcolor-indexed EQ zcl_xslx2_style_color=>c_indexed_sys_foreground.
...
* STEP 4: Create chart
    ...
    DATA lo_chartb TYPE REF TO zcl_xslx2_graph_bars.
    DATA lo_chartp TYPE REF TO zcl_xslx2_graph_pie.
    DATA lo_chartl TYPE REF TO zcl_xslx2_graph_line.
    DATA lo_chart TYPE REF TO zcl_xslx2_graph.

    DATA ls_serie TYPE zcl_xslx2_graph=>s_series.
    DATA ls_ax TYPE zcl_xslx2_graph_bars=>s_ax.
    DATA lv_str TYPE string.

    "Identify chart type
    CASE io_drawing->graph_type.
      WHEN zcl_xslx2_drawing=>c_graph_bars.
        lo_chartb ?= io_drawing->graph.
      WHEN zcl_xslx2_drawing=>c_graph_pie.
        lo_chartp ?= io_drawing->graph.
      WHEN zcl_xslx2_drawing=>c_graph_line.
    ...
    CASE io_drawing->graph_type.
      WHEN zcl_xslx2_drawing=>c_graph_bars.
        ...
        ENDLOOP.
        "endseries
        IF lo_chartb->ns_groupingval = zcl_xslx2_graph_bars=>c_groupingval_stacked.
        ...
        LOOP AT lo_chartb->axes INTO ls_ax.
          ...
          lo_element4->set_attribute_ns( name  = 'val'
                                  value = ls_ax-axid ).
          CASE ls_ax-type.
            WHEN zcl_xslx2_graph_bars=>c_catax.
            ...
            WHEN zcl_xslx2_graph_bars=>c_valax.
      ...
      WHEN zcl_xslx2_drawing=>c_graph_line.
        ...
        LOOP AT lo_chartl->axes INTO ls_ax.
          ...
          lo_element4->set_attribute_ns( name  = 'val'
                                  value = ls_ax-axid ).
          CASE ls_ax-type.
            WHEN zcl_xslx2_graph_line=>c_catax.
              lo_element3 = lo_document->create_simple_element( name = lc_xml_node_catax
                                                     parent = lo_element2 ).
              lo_element4 = lo_document->create_simple_element( name = lc_xml_node_axid
                                                     parent = lo_element3 ).
              ...
              lo_element4->set_attribute_ns( name  = 'val'
                                             value = ls_ax-nomultilvllbl ).
            WHEN zcl_xslx2_graph_line=>c_valax.
              lo_element3 = lo_document->create_simple_element( name = lc_xml_node_valax
                                                     parent = lo_element2 ).
              lo_element4 = lo_document->create_simple_element( name = lc_xml_node_axid
                                                     parent = lo_element3 ).
...
* STEP 5: Create xstring stream
  ...
  METHOD create_xl_comments.
    DATA:
      ...
      lo_comments            TYPE REF TO zcl_xslx2_comments,
...
** Constant node name
    ...
    DATA: lo_document           TYPE REF TO if_ixml_document,
          ...
          lo_iterator           TYPE REF TO zcl_xslx2_collection_iterator,
          lo_drawings           TYPE REF TO zcl_xslx2_drawings,
          lo_drawing            TYPE REF TO zcl_xslx2_drawing.
...
** Constant node name
    ...
    DATA: lo_drawing      TYPE REF TO zcl_xslx2_drawing,
          ...
          lt_drawings     TYPE zxslx2_t_drawings.

    FIELD-SYMBOLS: <fs_temp>     LIKE LINE OF lt_temp,
                   <fs_drawings> TYPE zxslx2_s_drawings.
...
** Constant node name
    ...
    DATA: lo_iterator     TYPE REF TO zcl_xslx2_collection_iterator,
          lo_drawing      TYPE REF TO zcl_xslx2_drawing,
          lo_document     TYPE REF TO if_ixml_document,
          lo_element_root TYPE REF TO if_ixml_element,
          lo_element      TYPE REF TO if_ixml_element,
...
* STEP 4: Create subnodes
    ...
    WHILE lo_iterator->has_next( ) EQ abap_true.
      lo_drawing ?= lo_iterator->get_next( ).
      IF lo_drawing->get_type( ) = zcl_xslx2_drawing=>type_image_header_footer.
        ADD 1 TO lv_relation_id.
...
** Constant node name
    ...
    DATA: lo_element_graphicframe TYPE REF TO if_ixml_element,
          ...
          ls_position             TYPE zxslx2_drawing_position,
...
** Constant node name
    ...
    DATA: lo_document              TYPE REF TO if_ixml_document,
          ...
          lv_str_column            TYPE zxslx2_cell_column_alpha,
...
* STEP 4: Create v:shapetype
    ...
    WHILE lo_iterator->has_next( ) EQ abap_true.
      ...
      zcl_xslx2_common=>convert_columnrow2column_a_row( EXPORTING i_columnrow = lo_comment->get_ref( )
                                                        IMPORTING e_column = lv_str_column
                                                                  e_row    = lv_row ).
      lv_column = zcl_xslx2_common=>convert_column2int( lv_str_column ).
...
** Constant node name
    ...
    DATA: lt_cell_data       TYPE zxslx2_t_cell_data_unsorted,
          lt_cell_data_rtf   TYPE zxslx2_t_cell_data_unsorted,
          lv_value           TYPE string,
          ls_shared_string   TYPE zxslx2_s_shared_string,
    ...
    FIELD-SYMBOLS: <fs_sheet_content> TYPE zxslx2_s_cell_data,
                   <fs_rtf>           TYPE zxslx2_s_rtf,
                   <fs_sheet_string>  TYPE zxslx2_s_shared_string.
...
* STEP 5: Create xstring stream
  ...
  METHOD create_xl_sheet_column_formula.
    ...
    TYPES: ls_column_formula_used     TYPE mty_column_formula_used,
           ...
           lv_top_cell_coords         TYPE zxslx2_cell_coords,
           lv_bottom_cell_coords      TYPE zxslx2_cell_coords,
           lv_cell_coords             TYPE zxslx2_cell_coords,
    ...
    DATA: lv_value                   TYPE string,
          ...
          lv_ref_value               TYPE string,
          lv_1st_line_shared_formula TYPE abap_bool.
    FIELD-SYMBOLS: <ls_column_formula>      TYPE zcl_xslx2_worksheet=>mty_s_column_formula,
                   <ls_column_formula_used> TYPE mty_column_formula_used.
    ...
    IF lv_1st_line_shared_formula = abap_true OR <ls_column_formula_used>-t <> 'shared'.
      lv_column_alpha = zcl_xslx2_common=>convert_column2alpha( ip_column = is_sheet_content-cell_column ).
      ...
      ELSE.
        ...
        lv_value = zcl_xslx2_common=>shift_formula(
  ...
  METHOD create_xl_sheet_ignored_errors.
    ...
    FIELD-SYMBOLS: <ls_ignored_errors> TYPE zcl_xslx2_worksheet=>mty_s_ignored_errors.
  ...
  METHOD create_xl_sheet_pagebreaks.
    DATA: lo_pagebreaks     TYPE REF TO zcl_xslx2_worksheet_pagebreaks,
          lt_pagebreaks     TYPE zcl_xslx2_worksheet_pagebreaks=>tt_pagebreak_at,
...
** Constant node name
    ...
    DATA: lo_document     TYPE REF TO if_ixml_document,
          ...
          lo_iterator     TYPE REF TO zcl_xslx2_collection_iterator,
          lo_table        TYPE REF TO zcl_xslx2_table,
          lo_link         TYPE REF TO zcl_xslx2_hyperlink.
...
* STEP 5: Create xstring stream
  ...
  METHOD create_xl_sheet_sheet_data.
    ...
    CONSTANTS: lc_dummy_cell_content       TYPE zxslx2_s_cell_data-cell_value VALUE '})~~~ This is a dummy value for ABAP2XLSX and you should never find this in a real excelsheet Ihope'.
...
*        lts_row_dimensions     TYPE zexcel_t_worksheet_rowdimensio,
          ...
          lo_row_empty           TYPE REF TO zcl_xslx2_row,
          lts_row_outlines       TYPE zcl_xslx2_worksheet=>mty_ts_outlines_row,

          ls_last_row            TYPE zxslx2_s_cell_data,
          ls_style_mapping       TYPE zxslx2_s_styles_mapping,
...
*Build table to hold all table-areas attached to this sheet
    ...
    WHILE lo_iterator->has_next( ) EQ abap_true.
      lo_table ?= lo_iterator->get_next( ).
      ls_table_area-left   = zcl_xslx2_common=>convert_column2int( lo_table->settings-top_left_column ).
...
*--------------------------------------------------------------------*
          lv_style_guid = io_worksheet->zif_xslx2_sheet_properties~get_style( ).
...
** Constant node name
    ...
    DATA: lo_document        TYPE REF TO if_ixml_document,
          ...
          lo_sub_element_2   TYPE REF TO if_ixml_element,
          lo_iterator        TYPE REF TO zcl_xslx2_collection_iterator,
          lo_iterator2       TYPE REF TO zcl_xslx2_collection_iterator,
          lo_worksheet       TYPE REF TO zcl_xslx2_worksheet,
          lo_style_cond      TYPE REF TO zcl_xslx2_style_cond,
          lo_style           TYPE REF TO zcl_xslx2_style.


    DATA: lt_fonts          TYPE zxslx2_t_style_font,
          ls_font           TYPE zxslx2_s_style_font,
          lt_fills          TYPE zxslx2_t_style_fill,
          ls_fill           TYPE zxslx2_s_style_fill,
          lt_borders        TYPE zxslx2_t_style_border,
          ls_border         TYPE zxslx2_s_style_border,
          lt_numfmts        TYPE zxslx2_t_style_numfmt,
          ls_numfmt         TYPE zxslx2_s_style_numfmt,
          lt_protections    TYPE zxslx2_t_style_protection,
          ls_protection     TYPE zxslx2_s_style_protection,
          lt_alignments     TYPE zxslx2_t_style_alignment,
          ls_alignment      TYPE zxslx2_s_style_alignment,
          lt_cellxfs        TYPE zxslx2_t_cellxfs,
          ls_cellxfs        TYPE zxslx2_s_cellxfs,
          ls_styles_mapping TYPE zxslx2_s_styles_mapping,
          lt_colors         TYPE zxslx2_t_style_color_argb,
    ...
    TYPES: BEGIN OF ts_built_in_format,
             num_format TYPE zxslx2_number_format,
             id         TYPE i,
    ...
    FIELD-SYMBOLS: <ls_built_in_format> LIKE LINE OF lt_built_in_num_formats,
                   <ls_reader_built_in> LIKE LINE OF zcl_xslx2_style_number_format=>mt_built_in_num_formats.
...
* Prepare built-in number formats.
    LOOP AT zcl_xslx2_style_number_format=>mt_built_in_num_formats ASSIGNING <ls_reader_built_in>.
...
* Compress number formats
      ...
      "-----------
      IF ls_numfmt-numfmt NE zcl_xslx2_style_number_format=>c_format_date_std." and ls_numfmt-NUMFMT ne 'STD_NDEC'. " ALE Changes on going
        "---
...
* Use these instead of duplicating the entries here, since they seem to be language-dependant and adjust to user settings in excel
          ...
          ELSE.
            ...
            ENDIF.
            ADD zcl_xslx2_common=>c_excel_numfmt_offset TO ls_cellxfs-numfmtid. " Add OXML offset for custom styles
          ENDIF.
      ...
      ELSE.
        ls_cellxfs-applynumberformat    = 1.
        IF ls_numfmt-numfmt EQ zcl_xslx2_style_number_format=>c_format_date_std.
...
* Remap styles
    ...
    LOOP AT lt_numfmts INTO ls_numfmt.
      lo_element_numfmt = lo_document->create_simple_element( name   = lc_xml_node_numfmt
                                                              parent = lo_document ).
      lv_value = sy-tabix + zcl_xslx2_common=>c_excel_numfmt_offset.
      CONDENSE lv_value.
...
* begin of change issue #366 - missing conditional rules: top10, move dfx-styles to own method
          ...
          WHEN zcl_xslx2_style_cond=>c_rule_textfunction.
            me->create_dxf_style( EXPORTING
    ...
    lo_element->set_attribute_ns( name  = lc_xml_attr_defaulttablestyle
                                  value = zcl_xslx2_table=>builtinstyle_medium9 ).
    lo_element->set_attribute_ns( name  = lc_xml_attr_defaultpivotstyle
                                  value = zcl_xslx2_table=>builtinstyle_pivot_light16 ).
    lo_element_root->append_child( new_child = lo_element ).
...
* STEP 5: Create xstring stream
  ...
  METHOD create_xl_styles_color_node.
    ...
    CHECK is_color-rgb IS NOT INITIAL OR
          is_color-indexed <> zcl_xslx2_style_color=>c_indexed_not_set OR
          is_color-theme <> zcl_xslx2_style_color=>c_theme_not_set OR
    ...
    IF is_color-indexed <> zcl_xslx2_style_color=>c_indexed_not_set.
      lv_value = zcl_xslx2_common=>number_to_excel_string( is_color-indexed ).
      lo_sub_element->set_attribute_ns( name  = lc_xml_attr_indexed
                                        value = lv_value ).
    ENDIF.

    IF is_color-theme <> zcl_xslx2_style_color=>c_theme_not_set.
      lv_value = zcl_xslx2_common=>number_to_excel_string( is_color-theme ).
      lo_sub_element->set_attribute_ns( name  = lc_xml_attr_theme
                                        value = lv_value ).
    ENDIF.

    IF is_color-tint IS NOT INITIAL.
      lv_value = zcl_xslx2_common=>number_to_excel_string( is_color-tint ).
  ...
  METHOD create_xl_table.
    ...
    DATA: lo_document     TYPE REF TO if_ixml_document,
          ...
          ls_fieldcat     TYPE zxslx2_s_fieldcatalog.
...
* STEP 3: Create main node table
    ...
    lv_value = zcl_xslx2_common=>number_to_excel_string( ip_value = lv_id ).
    ...
    IF io_table->settings-table_name IS NOT INITIAL AND lv_match EQ 0.
      ...
      IF ( strlen( io_table->settings-table_name ) = 1 AND io_table->settings-table_name CO 'CcRr' )
         OR zcl_xslx2_common=>shift_formula(
...
* STEP 5: Create xstring stream
  ...
  METHOD create_xl_theme.
    DATA: lo_theme TYPE REF TO zcl_xslx2_theme.
...
** Constant node name
    ...
    DATA: lo_document       TYPE REF TO if_ixml_document,
          ...
          lo_sub_element    TYPE REF TO if_ixml_element,
          lo_iterator       TYPE REF TO zcl_xslx2_collection_iterator,
          lo_iterator_range TYPE REF TO zcl_xslx2_collection_iterator,
          lo_worksheet      TYPE REF TO zcl_xslx2_worksheet,
    ...
    DATA: lv_xml_node_ridx_id TYPE string,
          lv_value            TYPE string,
          lv_syindex          TYPE string,
          lv_active_sheet     TYPE zxslx2_active_worksheet.
...
* STEP 4: Create subnode
    ...
    IF me->excel->zif_xslx2_book_protection~protected EQ abap_true.
      lo_element = lo_document->create_simple_element( name   = lc_xml_node_workbookprotection
                                                       parent = lo_document ).
      lv_value = me->excel->zif_xslx2_book_protection~workbookpassword.
      IF lv_value IS NOT INITIAL.
        lo_element->set_attribute_ns( name  = lc_xml_attr_workbookpassword
                                      value = lv_value ).
      ENDIF.
      lv_value = me->excel->zif_xslx2_book_protection~revisionspassword.
      IF lv_value IS NOT INITIAL.
        lo_element->set_attribute_ns( name  = lc_xml_attr_revisionspassword
                                      value = lv_value ).
      ENDIF.
      lv_value = me->excel->zif_xslx2_book_protection~lockrevision.
      CONDENSE lv_value NO-GAPS.
      lo_element->set_attribute_ns( name  = lc_xml_attr_lockrevision
                                    value = lv_value ).
      lv_value = me->excel->zif_xslx2_book_protection~lockstructure.
      CONDENSE lv_value NO-GAPS.
      lo_element->set_attribute_ns( name  = lc_xml_attr_lockstructure
                                    value = lv_value ).
      lv_value = me->excel->zif_xslx2_book_protection~lockwindows.
      CONDENSE lv_value NO-GAPS.
...
* issue #365 - test if sheet exists - otherwise set active worksheet to 1
    ...
    WHILE lo_iterator->has_next( ) EQ abap_true.
      ...
      lo_sub_element->set_attribute_ns( name  = lc_xml_attr_sheetid
                                        value = lv_syindex ).
      IF lo_worksheet->zif_xslx2_sheet_properties~hidden EQ zif_xslx2_sheet_properties=>c_hidden.
        lo_sub_element->set_attribute_ns( name  = lc_xml_attr_state
                                          value = 'hidden' ).
      ELSEIF lo_worksheet->zif_xslx2_sheet_properties~hidden EQ zif_xslx2_sheet_properties=>c_veryhidden.
        lo_sub_element->set_attribute_ns( name  = lc_xml_attr_state
                                          value = 'veryHidden' ).
      ENDIF.
...
* STEP 5: Create xstring stream
  ...
  METHOD get_shared_string_index.
    ...
    DATA ls_shared_string TYPE zxslx2_s_shared_string.
  ...
  METHOD is_formula_shareable.
    ...
    IF ip_formula NA '!'.
      lv_test_shared = zcl_xslx2_common=>shift_formula(
  ...
  METHOD set_vml_shape_footer.
    ...
    DATA: lv_content_left         TYPE string,
          ...
          lv_value                TYPE string,
          ls_drawing_position     TYPE zxslx2_drawing_position.
...
* BODY
  ...
  METHOD zif_xslx2_writer~write_file.
    me->excel = io_excel.
