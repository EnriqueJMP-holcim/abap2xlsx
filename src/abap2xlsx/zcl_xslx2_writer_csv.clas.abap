CLASS zcl_xslx2_writer_csv DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
*"* do not include other source files here!!!
  PRIVATE SECTION.
    ...
    CLASS-DATA worksheet_name TYPE zxslx2_worksheets_name .
    CLASS-DATA worksheet_index TYPE zxslx2_active_worksheet .
...
CLASS zcl_xslx2_writer_csv IMPLEMENTATION.
...
* End of insertion # issue 1134 - Dateretention of cellstyles(issue #139)
  ...
  METHOD create_csv.
    ...
    DATA: lo_autofilter TYPE REF TO zcl_xslx2_autofilter.
    ...
    DATA: ls_numfmt TYPE zxslx2_s_style_numfmt,
...
* STEP 1: Collect strings from the first worksheet
    ...
    DATA: current_worksheet_title TYPE zxslx2_sheet_title.
...
* --- Add empty rows
      WHILE lv_row < <fs_sheet_content>-cell_row.
        CONCATENATE lv_string zcl_xslx2_writer_csv=>eol INTO lv_string.
        lv_row = lv_row + 1.
        lv_col = 1.
      ENDWHILE.
...
* --- Add empty columns
      WHILE lv_col < <fs_sheet_content>-cell_column.
        CONCATENATE lv_string zcl_xslx2_writer_csv=>delimiter INTO lv_string.
        lv_col = lv_col + 1.
      ENDWHILE.
...
* ----- Use format to determine the data type and display format.
      CASE <fs_sheet_content>-data_type.
        ...
        WHEN 'd' OR 'D'.
          ...
          ELSE.
            lc_value = zcl_xslx2_common=>excel_string_to_date( ip_value = <fs_sheet_content>-cell_value ).
            TRY.
                lv_date = lc_value.
        ...
        WHEN 't' OR 'T'.
          lc_value = zcl_xslx2_common=>excel_string_to_time( ip_value = <fs_sheet_content>-cell_value ).
      ...
      CONCATENATE zcl_xslx2_writer_csv=>enclosure zcl_xslx2_writer_csv=>enclosure INTO lv_tmp.
      CONDENSE lv_tmp.
      REPLACE ALL OCCURRENCES OF zcl_xslx2_writer_csv=>enclosure IN lc_value WITH lv_tmp.

      FIND FIRST OCCURRENCE OF zcl_xslx2_writer_csv=>delimiter IN lc_value.
      IF sy-subrc = 0.
        CONCATENATE lv_string zcl_xslx2_writer_csv=>enclosure lc_value zcl_xslx2_writer_csv=>enclosure INTO lv_string.
      ELSE.
  ...
  METHOD set_enclosure.
    zcl_xslx2_writer_csv=>enclosure = ip_value.
  ENDMETHOD.


  METHOD set_endofline.
    zcl_xslx2_writer_csv=>eol = ip_value.
  ENDMETHOD.
