CLASS zcl_xslx2_writer_xlsm DEFINITION
  PUBLIC
  INHERITING FROM zcl_xslx2_writer_2007
  CREATE PUBLIC .
...
CLASS zcl_xslx2_writer_xlsm IMPLEMENTATION.
...
* Add vbaProject.bin to zip
    io_zip->add( name    = me->c_xl_vbaproject
                 content = me->excel->zif_xslx2_book_vba_project~vbaproject ).
...
* STEP 2: modify XML adding the vbaProject relation
    ...
    WHILE lo_element IS BOUND.
      lo_element->set_attribute_ns( name  = lc_xml_attr_codename
                                    value = io_worksheet->zif_xslx2_sheet_vba_project~codename_pr ).
      lo_element ?= lo_iterator->get_next( ).
    ENDWHILE.
...
* STEP 2: modify XML adding the vbaProject relation
    ...
    WHILE lo_element IS BOUND.
      lo_element->set_attribute_ns( name  = lc_xml_attr_codename
                                    value = me->excel->zif_xslx2_book_vba_project~codename ).
      lo_element ?= lo_iterator->get_next( ).
    ENDWHILE.
    ...
    WHILE lo_element IS BOUND.
      lo_element->set_attribute_ns( name  = lc_xml_attr_codename
                                    value = me->excel->zif_xslx2_book_vba_project~codename_pr ).
      lo_element ?= lo_iterator->get_next( ).
    ENDWHILE.
